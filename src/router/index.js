...
const mainRoutes = {
  ...
  children: [
    // 通過meta物件設定路由展示方式
    // 1. isTab: 是否通過tab展示内容, true: 是, false: 否
    // 2. iframeUrl: 是否通過iframe嵌套展示内容, '以http[s]://開頭': 是, '': 否
    // 提示: 如需要通過iframe嵌套展示内容, 但不通過tab打開, 請自行創建組件使用iframe處理!
...
router.beforeEach((to, from, next) => {
  ...
  if (router.options.isAddDynamicMenuRoutes || fnCurrentRouteType(to, globalRoutes) === 'global') {
    ...
    http({
      ...
      console.log(`%c${e} 請求選單列表和權限失败，跳轉至登入頁！！`, 'color:blue')
...
function fnAddDynamicMenuRoutes(menuList = [], routes = []) {
  ...
  if (temp.length >= 1) {
    ...
    // console.log('\n')
    // console.log('%c!<-------------------- 動態(選單)路由 s -------------------->', 'color:blue')
    // console.log(mainRoutes.children)
    // console.log('%c!<-------------------- 動態(選單)路由 e -------------------->', 'color:blue')
  }
}
