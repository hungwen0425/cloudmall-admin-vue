    ...
    return {
      ...
      dataRule: {
        ...
        paymentWay: [
          {
            required: true,
            message: "付款方式【 1:在線付款 2:貨到付款】不能為空",
            trigger: "blur"
          }
        ],
        taskStatus: [
          { required: true, message: "任務狀態不能為空", trigger: "blur" }
        ],
        orderBody: [
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/ware/wareordertask/info/${this.dataForm.id}`
            ),
            method: "get",
            ...
            if (data && data.code === 0) {
              ...
              this.dataForm.deliveryAddress =
                data.wareOrderTask.deliveryAddress;
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/ware/wareordertask/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
