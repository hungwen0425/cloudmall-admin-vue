          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              this.dataForm.flashOrderOvertime =
                data.orderSetting.flashOrderOvertime;
              this.dataForm.normalOrderOvertime =
                data.orderSetting.normalOrderOvertime;
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/order/ordersetting/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
