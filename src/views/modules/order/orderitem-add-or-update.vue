<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="商品sku編號" prop="skuId">
        <el-input v-model="dataForm.skuId" placeholder="商品sku編號"></el-input>
      </el-form-item>
      ...
      <el-form-item label="商品購買的數量" prop="skuQuantity">
        <el-input v-model="dataForm.skuQuantity" placeholder="商品購買的數量"></el-input>
      </el-form-item>
      ...
      <el-form-item label="該商品經過優惠後的分解金额" prop="realAmount">
        <el-input v-model="dataForm.realAmount" placeholder="該商品經過優惠後的分解金额"></el-input>
      </el-form-item>
      <el-form-item label="赠送積分" prop="giftIntegration">
        <el-input v-model="dataForm.giftIntegration" placeholder="赠送積分"></el-input>
      </el-form-item>
      <el-form-item label="赠送成長值" prop="giftGrowth">
        <el-input v-model="dataForm.giftGrowth" placeholder="赠送成長值"></el-input>
      </el-form-item>
    </el-form>
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        spuName: [
          { required: true, message: "spu_name不能為空", trigger: "blur" }
        ],
        spuPic: [
          { required: true, message: "spu_pic不能為空", trigger: "blur" }
        ],
        spuBrand: [
        ...
        skuId: [
          { required: true, message: "商品sku編號不能為空", trigger: "blur" }
        ],
        skuName: [
        ...
        skuQuantity: [
          { required: true, message: "商品購買的數量不能為空", trigger: "blur" }
        ],
        skuAttrsVals: [
          {
            required: true,
            message: "商品銷售屬性組合（JSON）不能為空",
            trigger: "blur"
          }
        ],
        promotionAmount: [
        ...
        realAmount: [
          {
            required: true,
            message: "該商品經過優惠後的分解金额不能為空",
            trigger: "blur"
          }
        ],
        giftIntegration: [
          { required: true, message: "赠送積分不能為空", trigger: "blur" }
        ],
        giftGrowth: [
          { required: true, message: "赠送成長值不能為空", trigger: "blur" }
        ]
      }
    };
  },
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/order/orderitem/info/${this.dataForm.id}`
            ),
            method: "get",
            ...
            if (data && data.code === 0) {
              ...
              this.dataForm.integrationAmount =
                data.orderItem.integrationAmount;
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/order/orderitem/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
